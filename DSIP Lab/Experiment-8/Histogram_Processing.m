% == == == == == == == == == == == == == == == == == == == == == == == == == ==
    == == == == == == == ==
    = % DIGITAL SIGNAL AND IMAGE PROCESSING LAB % % Experiment No. : 8 %
              Experiment Title
    : Histogram Processing of an Image %
      % Aim : % Write a program to implement Histogram Processing.% %
              Student Details : % Name : Amey Thakur % Roll No. : 50 %
                                         Class : COMPS -
          BE - B -
          50 % Batch : B3 % % Date of Experiment : 15 / 09 / 2021 %
    ...
    =
        ...
        % Clear workspace,
    close all figures, and clear command window close all;
clear all;
clc;
...
for i = 1:255
    for j = 1:255
        z = p(i,j);
% Pixel intensity value h(z + 1) = h(z + 1) + 1;
% Increment corresponding bin end end

    % Display manually computed histogram figure;
stem(h);
grid on;
